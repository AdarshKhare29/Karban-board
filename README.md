...
A full-stack **collaborative Kanban board** with real-time updates, authentication, board roles, comments, per-card activity timeline, and online presence.
...
3. Confirm resources from blueprint:
   - `kanban-db` (PostgreSQL)
   - `kanban-api` (Node web service)
   - `kanban-web` (static web app)

4. Click deploy.

5. After first deploy:
   - Open `kanban-api` -> `Shell` and run:
     ```bash
     npm run db:init
     ```
   - Verify logs show `Database schema initialized.`
   - Open `kanban-api/api/health` and confirm `{ "ok": true }`.
   - Open `kanban-web` URL and test login + realtime updates.

### Notes
- `JWT_SECRET` is auto-generated by Render from `render.yaml`.
- `CLIENT_ORIGIN`, `VITE_API_URL`, and `VITE_SOCKET_URL` are auto-linked between services.
- Free plans may sleep after inactivity, so first request can be slow.
- On free tier, Blueprint `pre-deploy` hooks are not supported, so run `npm run db:init` manually once after first deploy.
